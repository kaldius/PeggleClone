/**
 `LevelDataManager` helps provide one point where all level data can be saved and loaded from Core Data.
 */
...
struct LevelDataManager {
    ...
    func fetchLevelData(levelName: String) throws -> LevelData {
        ...
        guard levelDatas.count > 0 else {
            throw PeggleError.levelDoesNotExistError(levelName: levelName)
        }
        guard levelDatas.count == 1 else {
            throw PeggleError.duplicateLevelNameError(levelName: levelName)
        }
        return levelDatas[0]
    }
    ...
    func saveLevelData(level: Level, overwrite: Bool = false) throws {
        guard let matchingLevelData = try? fetchAllLevelDataMatching(levelName: level.name) else { return }
        let containsDuplicate = !matchingLevelData.isEmpty
        if containsDuplicate && !overwrite {
            throw PeggleError.duplicateLevelNameError(levelName: level.name)
