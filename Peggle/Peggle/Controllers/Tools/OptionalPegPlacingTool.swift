/**
 The `OptionalPegPlacingTool` represents the tool in the level designer that allows the user
 to put down an optional peg.
 */

import CoreGraphics
import UIKit

struct OptionalPegPlacingTool: GameObjectPlacingTool {
    ...
    func useOnLevelView(withGesture gestureRecognizer: UITapGestureRecognizer) {
        ...
        let newOptionalPeg = Peg(center: gestureLocation,
                                 radius: Peg.DEFAULTPEGRADIUS,
                                 type: .optional)
        // Adding a peg is allowed to fail if tap is on invalid area
        try? delegate.add(gameObject: newOptionalPeg)
    }
    ...
    static func == (lhs: OptionalPegPlacingTool, rhs: OptionalPegPlacingTool) -> Bool {
